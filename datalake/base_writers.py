...
class AzureConnectionStringAuthenticator:
    ...
    def authenticate(self, configs: dict) -> BlobServiceClient:
        ...
        connect_str: str = ""
        if not os.getenv("AZURE_STORAGE_CONNECTION_STRING") or not configs.get(
...
class AzureAuthenticator:
    ...
    def authenticate(self, configs):
        """Azure Cloud Storage Authenticator"""
        if configs.get("AZURE_STORAGE_CONNECTION_STRING"):
            return AzureConnectionStringAuthenticator().authenticate(configs=configs)
        return AzureDefaultAuthenticator().authenticate(configs=configs)
